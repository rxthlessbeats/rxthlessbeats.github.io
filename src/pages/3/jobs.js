...
const choices = ["餐飲零售業", "科技業", "時薪", "月薪", "台北", "桃園", "新竹", "台中", "台南", "高雄"]; // 0~9
...
function JobsPage () {
    ...
    const deleteFilter = (idx) => {
        ...
        if (idx === 2 || idx === 3) {
            ...
            if (allFalse) for(i=4; i<=9; i++) newArr[i] = true;
            else newArr[idx] = false;
        } else {
            let allFalse = true;
            let from = 10, end = filter.length-1;
            if (idx === 0) from = 15;   
            else if (idx === 1) end = 14;
            for (i=from; i <= end; i++) {
                if (i === idx) continue;
                if (filter[i]) {
                    allFalse = false;
            ...
            if (allFalse) {
                for (i=10; i<filter.length; i++) newArr[i] = true;
                newArr[0] = newArr[1] = true;
            } else {
                if (idx === 0) for (i=10; i<=14; i++) newArr[i] = false;
                else if (idx === 1) for (i=15; i<filter.length; i++) newArr[i] = false;
                newArr[idx] = false;
            }
        }
        setFilter(newArr);
    ...
    const filter_valid = (idx) => {
        ...
        } else if (idx >= 4 && idx <= 9) { // places
            if (filter[4] && filter[5] && filter[6] && filter[7] && filter[8] && filter[9]) return false;
        } else if (idx >= 10 && idx <= 14) { // food
    ...
    const filter_true = (j) => {
        // job_type
        let job_type_idx = -1;
        for (var i=10; i<all_filter.length; i++) {
